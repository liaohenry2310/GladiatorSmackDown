...
namespace GladiatorGame
{
    public class NavigationGraph : MonoBehaviour
    {
        ...
        public Vector2[] GetPath(Vector2 start, Vector2 end)
        {
            ...
            var source = tilemap.WorldToCell(start.ToVector3());
            ...
            while(!found && openList.Count > 0)
            {
                ...
                {
                    Vector3Int[] neighbors = GetNeighbors(nextTile);
        ...
        private List<Vector2> StraightenPath(List<Vector2> path)
        {
            ...
            {
                RaycastHit2D hit = Physics2D.Linecast(path[prev], path[curr], ~Physics2D.IgnoreRaycastLayer);
